...
bool is_collinear(struct point p1, struct point p2, struct point p3) {
    return is_zero((p2.x * p3.y - p2.y * p3.x) + (p3.x * p1.y - p3.y * p1.x) + (p1.x * p2.y - p1.y * p2.x));
}
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (is_zero(q.y - s.y)) { 
            ...
            if (is_zero(q.y - t.y)) { // check four points
                ...
                if (s.x > q.x) {
                    if (((prev.y > q.y) && (next.y < q.y)) || ((prev.y < q.y) && (next.y > q.y))) {
                        ++num_crossings;
                    }
                }
...
void check_list(const struct list *ql, const struct list * pl) {
    for (int i = 0; i < ql->sz; ++i) check_inclusion(ql->pts[i], pl);
}
